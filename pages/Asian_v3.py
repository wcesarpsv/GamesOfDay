...
def _evaluate_threshold_side(df, side, thr):
    """
    df   : DataFrame com VG_HOME / VG_AWAY (em eixo HOME)
    side : 'HOME' ou 'AWAY'
...
#   Usando VG_HOME e VG_AWAY já no eixo HOME
# ===============================================================
def find_league_thresholds(history: pd.DataFrame, min_bets: int = 60):
    ...
    # === Classificar ligas por estabilidade (TIER_1 / 2 / 3) ===
    league_stability = {}
    for lg in leagues:
    ...
    # ===== Fallback global (caso liga tenha poucos jogos) =====
    global_tier = "TIER_2"
    ...
    for lg in leagues:
        ...
        # Se a liga tem poucos jogos, aproximar thresholds do global
        if n_games < min_bets:
            alpha = n_games / max(min_bets, 1)  # 0 → totalmente global, 1 → totalmente local
            def blend(local, global_v):
                return round(alpha * local + (1 - alpha) * global_v, 3)
            ...
            # Liga com amostra razoável: usa thresholds do TIER diretamente
            out[lg] = {
                "TIER": tier,
                "HOME":        base["HOME"],
                "AWAY":        base["AWAY"],
...
def main_calibrado():
    ...
    if st.button("🚀 Executar Análise DUAL", type="primary"):
        with st.spinner("Treinando modelos..."):
            ...
            # ===== HISTÓRICO PARA CALIBRAR THRESHOLDS (APENAS JOGOS COM FT) =====
            ...
            # ===== PREVER O HISTÓRICO – MODELO HOME (EIXO HOME) =====
            ...
            # ===== PREVER O HISTÓRICO – MODELO AWAY (DEPOIS CONVERTER PARA EIXO HOME) =====
            pred_away_hist_raw = _predict_block(
                hist_for_pred,
                features_3d_common,
                scaler_away,
                modelo_away_reg,
            ...
            # ===== ARMAZENAR PREDIÇÕES NO HISTÓRICO =====
            hist_for_pred['Pred_HOME'] = pred_home_hist                   # já no eixo HOME
            hist_for_pred['Pred_AWAY_RAW'] = pred_away_hist_raw           # eixo AWAY nativo
            hist_for_pred['Pred_AWAY_HOME_AXIS'] = -pred_away_hist_raw    # convertido p/ eixo HOME
    
            # ===== VALUE GAPS – TUDO NO EIXO HOME! =====
            # Asian_Line_Decimal sempre = linha do HOME
            ...
            # OBS: VG_AWAY aqui já está no eixo HOME (Pred_AWAY convertido).
    
            # ===== CALCULAR THRESHOLDS POR LIGA (USANDO VG_HOME e VG_AWAY) =====
