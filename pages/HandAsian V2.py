...
# ================================================================
# 📡 LIVE SCORE MONITOR – SISTEMA 3D UNIFICADO (AH + 1x2)
# ================================================================
st.markdown("## 📡 Live Score Monitor – Sistema 3D (AH + 1x2)")

# ============================================================
# ⚙️ Funções auxiliares 1X2
# ============================================================
def determine_match_result_1x2(row):
    """Determina resultado 1X2 (Home/Away/Draw)."""
    try:
        gh = float(row['Goals_H_Today']) if pd.notna(row['Goals_H_Today']) else np.nan
        ga = float(row['Goals_A_Today']) if pd.notna(row['Goals_A_Today']) else np.nan
    except (ValueError, TypeError):
...
def check_recommendation_correct_1x2(recomendacao, match_result):
    """Verifica se a recomendação acertou no 1X2."""
    if pd.isna(recomendacao) or match_result is None or '⚖️ ANALISAR' in str(recomendacao).upper():
        return None

    rec = str(recomendacao).upper()
    is_home_bet = any(k in rec for k in ['HOME', '→ HOME', 'FAVORITO HOME', 'VALUE NO HOME', 'MODELO CONFIA HOME'])
    is_away_bet = any(k in rec for k in ['AWAY', '→ AWAY', 'FAVORITO AWAY', 'VALUE NO AWAY', 'MODELO CONFIA AWAY'])
...
def calculate_profit_1x2(recomendacao, match_result, odds_row):
    """Calcula o lucro líquido baseado no resultado 1X2."""
    if pd.isna(recomendacao) or match_result is None or '⚖️ ANALISAR' in str(recomendacao).upper():
        return 0
...
def update_real_time_data_1x2(df):
    """Atualiza colunas do sistema 1X2."""
    df = df.copy()
    df['Result_1x2'] = df.apply(determine_match_result_1x2, axis=1)
    df['Quadrante_Correct_1x2'] = df.apply(lambda r: check_recommendation_correct_1x2(r['Recomendacao'], r['Result_1x2']), axis=1)
    df['Profit_1x2'] = df.apply(lambda r: calculate_profit_1x2(r['Recomendacao'], r['Result_1x2'], r), axis=1)
    return df


# ============================================================
# ⚙️ Funções Handicap Asiático (3D)
# ============================================================
def determine_handicap_result_3d(row):
    """Determina resultado do Handicap Asiático (suporta half-win/half-loss)."""
    try:
        gh = float(row['Goals_H_Today']) if pd.notna(row['Goals_H_Today']) else np.nan
        ga = float(row['Goals_A_Today']) if pd.notna(row['Goals_A_Today']) else np.nan
    ...
    adjusted = (gh + asian_line - ga) if side == "HOME" else (ga - asian_line - gh)
    if adjusted > 0:
        return f"{side}_COVERED"
    elif adjusted < 0:
        return f"{'AWAY' if side == 'HOME' else 'HOME'}_COVERED"
...
def check_handicap_recommendation_correct_3d(recomendacao, handicap_result):
    """Verifica se a recomendação AH foi correta."""
    if pd.isna(recomendacao) or handicap_result is None or '⚖️ ANALISAR' in str(recomendacao).upper():
        return None

    rec = str(recomendacao).upper()
    is_home_bet = any(k in rec for k in ['HOME', 'VALUE NO HOME', 'FAVORITO HOME'])
    is_away_bet = any(k in rec for k in ['AWAY', 'VALUE NO AWAY', 'FAVORITO AWAY'])
...
def calculate_handicap_profit_3d(recomendacao, handicap_result, odds_row):
    """Calcula lucro líquido com suporte a half-win / half-loss."""
    if pd.isna(recomendacao) or handicap_result is None or '⚖️ ANALISAR' in str(recomendacao).upper():
        return 0
    ...
    if (is_home_bet and handicap_result == "HOME_COVERED") or (is_away_bet and handicap_result == "AWAY_COVERED"):
        return odd
...
def update_real_time_data_3d(df):
    """Atualiza colunas do Handicap AH (3D)."""
    df = df.copy()
    df['Handicap_Result'] = df.apply(determine_handicap_result_3d, axis=1)
    df['Quadrante_Correct'] = df.apply(lambda r: check_handicap_recommendation_correct_3d(r['Recomendacao'], r['Handicap_Result']), axis=1)
    df['Profit_Quadrante'] = df.apply(lambda r: calculate_handicap_profit_3d(r['Recomendacao'], r['Handicap_Result'], r), axis=1)
    return df
...
# ============================================================
# 🧾 SUMÁRIOS DE PERFORMANCE
# ============================================================
def generate_live_summary_3d(df):
    """Resumo dos resultados do Handicap (3D)."""
    finished = df[df['Handicap_Result'].notna()]
    if finished.empty:
        return {"Total Jogos": len(df), "Jogos Finalizados": 0, "Winrate AH": "0%", "Profit AH": "0.00u", "ROI AH": "0%"}
    bets = finished[finished['Quadrante_Correct'].notna()]
    total, correct = len(bets), bets['Quadrante_Correct'].sum()
    profit, roi = bets['Profit_Quadrante'].sum(), (bets['Profit_Quadrante'].sum() / total if total else 0)
    winrate = (correct / total) * 100 if total > 0 else 0
    return {
        "Total Jogos": len(df),
        "Jogos Finalizados": len(finished),
        "Winrate AH": f"{winrate:.1f}%",
        "Profit AH": f"{profit:.2f}u",
        "ROI AH": f"{roi*100:.1f}%"
    }

# Exibir resumo AH
st.markdown("### 🎯 Sistema Handicap Asiático (3D)")
st.json(generate_live_summary_3d(ranking_quadrantes))

# ============================================================
# 🎯 SISTEMA 1X2 – LIVE SUMMARY
# ============================================================
st.markdown("### ⚽ Sistema 1X2 (Resultado Final)")
...
def compare_systems_summary(df):
    """Compara o desempenho do sistema AH e 1x2."""
