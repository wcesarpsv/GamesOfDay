...
def calcular_distancias_3d(df):
    """Calcula distância 3D, separação e ângulos usando Aggression, HandScore e Momentum"""
    df = df.copy()
    
    # Verificar se as colunas necessárias existem
    required_cols = ['Aggression_Home', 'Aggression_Away', 'HandScore_Home', 'HandScore_Away', 'M_H', 'M_A']
    missing_cols = [col for col in required_cols if col not in df.columns]
    
    if missing_cols:
        st.warning(f"⚠️ Colunas faltando para cálculo 3D: {missing_cols}")
        df['Quadrant_Dist_3D'] = np.nan
        df['Quadrant_Separation_3D'] = np.nan
        df['Quadrant_Angle_XY'] = np.nan
        df['Quadrant_Angle_XZ'] = np.nan
        df['Quadrant_Angle_YZ'] = np.nan
        df['Momentum_Diff'] = np.nan
        return df

    # Diferenças nas 3 dimensões
    dx = df['Aggression_Home'] - df['Aggression_Away']  # X: Aggression
    dy = df['HandScore_Home'] - df['HandScore_Away']    # Y: HandScore  
    dz = df['M_H'] - df['M_A']                          # Z: Momentum

    # Distância Euclidiana 3D com scaling adaptativo
    df['Quadrant_Dist_3D'] = np.sqrt(
        dx**2 +                    # Aggression (já normalizada -1 a 1)
        (dy/60)**2 * 2.5 +         # HandScore (-60 a 60) - escala ajustada
        (dz/3.5)**2 * 1.5          # Momentum (-3.5 a 3.5) - escala ajustada
    ) * 10

    # Ângulos nos diferentes planos
    df['Quadrant_Angle_XY'] = np.degrees(np.arctan2(dy, dx))      # Plano tradicional Aggression × HandScore
    df['Quadrant_Angle_XZ'] = np.degrees(np.arctan2(dz, dx))      # Plano Aggression × Momentum
    df['Quadrant_Angle_YZ'] = np.degrees(np.arctan2(dz, dy))      # Plano HandScore × Momentum

    # Separação ponderada 3D (incorporando todas as dimensões)
    df['Quadrant_Separation_3D'] = 0.4 * dy + 0.4 * (60 * dx) + 0.2 * (20 * dz)
    
    # Diferença de momentum
    df['Momentum_Diff'] = dz
    
    # Magnitude vetorial 3D
