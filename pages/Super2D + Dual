...
def build_hcapzone_tables_confronto(history: pd.DataFrame,
                                    min_line: float = -1.5,
                                    max_line: float = 1.5) -> dict:
    """
    HcapZone v2 – por CONFRONTO de quadrantes.

    Calcula:
      P(cobrir) para:
        - HOME: (Quadrante_Home, Quadrante_Away, Asian_Line_Bin)
        - AWAY: (Quadrante_Away, Quadrante_Home, Asian_Line_Bin)

    Push continua valendo 0 (win=1, push=0, loss=0).
    """
    ...
    # Só jogos completos e com info de quadrante + linha
    df = df.dropna(subset=[
        'Goals_H_FT', 'Goals_A_FT',
        'Asian_Line_Decimal',
        'Quadrante_Home', 'Quadrante_Away'
    ...
    # Range de linhas onde você confia nas estatísticas
    df = df[
        (df['Asian_Line_Decimal'] >= min_line) &
        (df['Asian_Line_Decimal'] <= max_line)
    ].copy()
    ...
    # ================= HOME – quadrante da casa vs quadrante do visitante ===============
    if 'League' in df.columns:
        league_home = (
            df.groupby(['League', 'Quadrante_Home', 'Quadrante_Away', 'Asian_Line_Bin'])['Cover_Home']
            .agg(['mean', 'size'])
            .reset_index()
            .rename(columns={
                'mean': 'CoverRate',
        ...
        league_home = pd.DataFrame(columns=['League', 'Quadrante_Home', 'Quadrante_Away', 'Asian_Line_Bin', 'CoverRate', 'N'])

    global_home = (
        df.groupby(['Quadrante_Home', 'Quadrante_Away', 'Asian_Line_Bin'])['Cover_Home']
        .agg(['mean', 'size'])
        .reset_index()
        .rename(columns={
            'mean': 'CoverRate',
    ...
    # ================= AWAY – quadrante do visitante vs quadrante da casa ===============
    if 'League' in df.columns:
        league_away = (
            df.groupby(['League', 'Quadrante_Away', 'Quadrante_Home', 'Asian_Line_Bin'])['Cover_Away']
            .agg(['mean', 'size'])
            .reset_index()
            .rename(columns={
                'mean': 'CoverRate',
        ...
        league_away = pd.DataFrame(columns=['League', 'Quadrante_Away', 'Quadrante_Home', 'Asian_Line_Bin', 'CoverRate', 'N'])

    global_away = (
        df.groupby(['Quadrante_Away', 'Quadrante_Home', 'Asian_Line_Bin'])['Cover_Away']
        .agg(['mean', 'size'])
        .reset_index()
        .rename(columns={
            'mean': 'CoverRate',
...
def attach_hcapzone_score_confronto(df: pd.DataFrame,
    ...
    """
    HcapZone v2 – baseado no CONFRONTO:
    ...
    Mantém a mesma saída:
        HcapZone_Score (0–1), HcapZone_N, HcapZone_Source
    """
    ...
    def lookup_row(row):
        ...
        # Handicap da aposta já na perspectiva do lado (HOME: linha da casa, AWAY: linha invertida)
        line = row.get('AH_ML_Side', np.nan)
        if pd.isna(line):
            # fallback: se não tiver AH_ML_Side, usa Asian_Line_Decimal com sinal
            if 'Asian_Line_Decimal' in row:
                line = row['Asian_Line_Decimal'] if side == 'HOME' else -row['Asian_Line_Decimal']
            else:
        ...
        # range estatístico [-1.50, +1.50]
        if line < -1.5 or line > 1.5:
            return np.nan, np.nan, ""

        line_bin = round(line * 4) / 4

        # Quadrantes do lado e do oponente
        if side == 'HOME':
            q_lado = row.get('Quadrante_Home', np.nan)
            q_op = row.get('Quadrante_Away', np.nan)
            t_league = league_home
            t_global = global_home
            col_lado = 'Quadrante_Home'
            col_op = 'Quadrante_Away'
        else:
            q_lado = row.get('Quadrante_Away', np.nan)
            q_op = row.get('Quadrante_Home', np.nan)
            t_league = league_away
            t_global = global_away
            col_lado = 'Quadrante_Away'
            col_op = 'Quadrante_Home'

        if pd.isna(q_lado) or pd.isna(q_op):
            return np.nan, np.nan, ""

        # 1) Tenta tabela por LIGA
        if (
            use_league and
            league is not None and league != "" and
            not t_league.empty and
            'League' in t_league.columns
        ):
            mask_l = (
                (t_league['League'] == league) &
                (t_league[col_lado] == q_lado) &
                (t_league[col_op] == q_op) &
        ...
        # 2) Fallback GLOBAL (mesmo lado, mesmo oponente, mesma linha)
        if not t_global.empty:
            mask_g = (
                (t_global[col_lado] == q_lado) &
                (t_global[col_op] == q_op) &
...
def estilo_tabela_quadrantes_dual(df):
    ...
    def cor_hcapzone(val):
        ...
        if v >= 70:
            ...
            return 'background-color: #ffeb9c; color: #9c5700;'  # amarelo
        else:
    ...
    # HcapZone_Score com as faixas 70 / 55
    if 'HcapZone_Score' in df.columns:
        styler = styler.applymap(cor_hcapzone, subset=['HcapZone_Score'])
...
hcap_tables = build_hcapzone_tables_confronto(history)
...
if not games_today.empty and 'Quadrante_ML_Score_Home' in games_today.columns:
    ...
    # Anexar HcapZone_Score + AH_ML_Side
