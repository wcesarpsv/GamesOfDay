...
def to_net_odds(x):
    ...
    try:
        ...
        return None
    # odds decimais → lucro líquido por stake 1 = odds - 1
    return v - 1.0
...
for file in sorted(os.listdir(GAMES_FOLDER)):
    if file.endswith(".csv"):
        ...
        # mantém apenas jogos com gols + odds presentes
...
# escolha do lado ANTES do slider de AH (opção 2)
bet_on = st.sidebar.selectbox("🎯 Bet on", ["Home", "Away"])

# Diff_Power e Diff_HT_P (sempre iguais)
...
# 🔁 Opção 2: mapear a linha para o lado apostado
# Se Home → AH_clean_for_side = AH_clean; se Away → AH_clean_for_side = -AH_clean
...
# 🧮 Aplica filtros (usando AH_for_side)
filtered_df = df_all[
...
if not filtered_df.empty:
    ...
    mean_ah_home = filtered_df["AH_clean"].mean()  # linha do Home (para referência)
    mean_ah_side = filtered_df["AH_clean_for_side"].mean()  # linha no lado apostado
