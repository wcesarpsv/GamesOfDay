...
def determine_handicap_result_history(row, side="HOME"):
    ...
    if is_quarter:
        # decompõe linha quarter em duas half-lines equivalentes
        if asian_line > 0:
...
def calculate_handicap_profit_history(row, side="HOME"):
    ...
    if (side == "HOME" and result == "HOME_COVERED") or \
       (side == "AWAY" and result == "AWAY_COVERED"):
        return float(odd)           # win cheia
    elif result == "HALF_WIN":
        return float(odd) / 2.0     # half-win
    elif result == "HALF_LOSS":
        return -0.5
    elif result == "PUSH":
        return 0.0                  # ✅ push = 0
    else:
        return -1.0                 # loss cheia
...
def treinar_modelo_ev_3d(history, games_today):
    ...
    # Garantir features 3D + clusters já calculados
    history = calcular_distancias_3d(history)
    games_today = calcular_distancias_3d(games_today)
    ...
    # Edge = prob * odd líquida (esperança de retorno)
...
def calcular_confiabilidade_ligas(df):
    ...
    # Score composto (quanto maior, mais estável e previsível)
