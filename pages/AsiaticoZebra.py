...
# ============================================================
# 🔢 Z-SCORES M / MT (recalcular a partir do HandScore)
# ============================================================
...
def treinar_modelo_handicap_regressao_calibrado_v2(history, games_today):
    ...
    features = ['Quadrant_Dist_3D','Quadrant_Separation_3D','Vector_Sign',
...
def treinar_modelo_handicap_classificacao_calibrado_v2(history, games_today):
    ...
    map_cat = {'MODERATE_HOME': -0.75, 'LIGHT_HOME': -0.25, 'NEUTRAL': 0,
               'LIGHT_AWAY': +0.25, 'MODERATE_AWAY': +0.75}
    games_today['Handicap_Predito_Classificacao_Calibrado'] = \
        games_today['Handicap_Categoria_Predito_Calibrado'].map(map_cat)
...
def treinar_modelo_away_handicap_classificacao_calibrado(history, games_today):
    ...
    map_cat = {'MODERATE_AWAY': -0.75, 'LIGHT_AWAY': -0.25, 'NEUTRAL': 0,
               'LIGHT_HOME': +0.25, 'MODERATE_HOME': +0.75}
    games_today['Handicap_AWAY_Predito_Classificacao_Calibrado'] = \
        games_today['Handicap_Categoria_AWAY_Predito_Calibrado'].map(map_cat)
...
def _predict_block(history, features, scaler, model_reg, model_cls, label_enc):
    ...
    mnum = {'MODERATE_HOME': -0.75, 'LIGHT_HOME': -0.25, 'NEUTRAL': 0.0,
            'LIGHT_AWAY': +0.25, 'MODERATE_AWAY': +0.75,
            'MODERATE_AWAY': -0.75, 'LIGHT_AWAY': -0.25,
            'LIGHT_HOME': +0.25, 'MODERATE_HOME': +0.75}
...
def analisar_value_bets_dual_modelos(games_today: pd.DataFrame, league_thresholds: dict):
    ...
    for _, row in games_today.iterrows():
        ...
        pred_home = 0.7 * float(row.get('Handicap_Predito_Regressao_Calibrado', 0) or 0) + \
                    0.3 * float(row.get('Handicap_Predito_Classificacao_Calibrado', 0) or 0)
        pred_away = 0.7 * float(row.get('Handicap_AWAY_Predito_Regressao_Calibrado', 0) or 0) + \
                    0.3 * float(row.get('Handicap_AWAY_Predito_Classificacao_Calibrado', 0) or 0)
        ...
        # CENÁRIO 3: Linhas extremas (pedir convicção alta)
...
def plot_analise_dual_modelos(games_today: pd.DataFrame):
    ...
    ax1.axhline(y=0.15, linestyle='--', alpha=0.5, label='Value Threshold ~ liga/linha')
    ax1.axhline(y=-0.15, linestyle='--', alpha=0.5)
    ax1.set_xlabel('Jogos'); ax1.set_ylabel('Value Gap'); ax1.set_title('Value Gaps: HOME vs AWAY'); ax1.legend(); ax1.grid(True, alpha=0.3)

    handicaps_home, handicaps_away, asian_lines = [], [], []
    for _, row in games_today.iterrows():
        pred_home = 0.7 * float(row.get('Handicap_Predito_Regressao_Calibrado', 0) or 0) + \
                    0.3 * float(row.get('Handicap_Predito_Classificacao_Calibrado', 0) or 0)
        pred_away = 0.7 * float(row.get('Handicap_AWAY_Predito_Regressao_Calibrado', 0) or 0) + \
    ...
    ax2.scatter(asian_lines, handicaps_home, alpha=0.7, s=60, label='HOME Predito')
    ax2.scatter(asian_lines, handicaps_away, alpha=0.7, s=60, label='AWAY Predito')
    ax2.plot([-1.5, 1.5], [-1.5, 1.5], 'k--', alpha=0.3, label='Linha Mercado (y=x)')
    ax2.set_xlabel('Asian Line (Mercado)'); ax2.set_ylabel('Handicap Predito'); ax2.set_title('Predito vs Mercado'); ax2.legend(); ax2.grid(True, alpha=0.3)
...
def main_calibrado():
    ...
    if "Date" in history.columns:
        try:
            ...
            # Apenas jogos passados no histórico
            ...
            # Concatenar passado + hoje para calcular M / MT coerente com o estado da liga
            ...
            # fallback: calcular zscore separadamente
    ...
    # Limpar features 3D no histórico e hoje
    history[features_3d_common] = clean_features_for_training(history[features_3d_common])
    games_today[features_3d_common] = clean_features_for_training(games_today[features_3d_common])
    ...
    if st.button("🚀 Executar Análise DUAL", type="primary"):
        with st.spinner("Treinando modelos..."):
            ...
            pred_home_hist = _predict_block(hist_for_pred, features_3d_common_local, scaler_home, modelo_home_reg, modelo_home_cls, le_home)
            pred_away_hist = _predict_block(hist_for_pred, features_3d_common_local, scaler_away, modelo_away_reg, modelo_away_cls, le_away)
