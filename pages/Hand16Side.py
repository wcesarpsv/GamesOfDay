...
def adicionar_indicadores_explicativos_3d_16_dual(df):
    ...
    def gerar_recomendacao_3d_16_dual(row):
        ...
        elif momentum_h < -1.0 and score_away >= 0.65:
            return f'2 - AWAY → 🔻 HOME EM MOMENTUM NEGATIVO ({score_away:.1%})'
        elif momentum_a < -1.0 and score_home >= 0.65:
            return f'1 - HOME → 🔻 AWAY EM MOMENTUM NEGATIVO ({score_home:.1%})'
        elif 'Neutro' in home_q and score_away >= 0.58 and momentum_a > 18:
            return f'2 - 🔄 AWAY EM NEUTRO (+Momentum) ({score_away:.1%})'
        elif 'Neutro' in away_q and score_home >= 0.58 and momentum_h > 10:
