...
def load_and_filter_history(selected_date_str):
    ...
    if "Date" in history.columns:
        ...
        # Filtrar TEMPORALMENTE primeiro (CORREÇÃO CRÍTICA)
...
def create_better_target_corrigido(df):
    ...
    df["Target_AH_3Class"] = np.select(conditions, choices, default=1)
    
    # Para modelo binário, filtrar apenas casos claros
...
def create_robust_features(df):
    ...
    df['Momentum_Advantage'] = (df['M_H'] - df['M_A']) + (df['MT_H'] - df['MT_A'])
...
def train_improved_model(X, y, feature_names):
    ...
    # Parâmetros otimizados para betting
    model = RandomForestClassifier(
        n_estimators=200,           # Reduzir overfitting
        max_depth=10,               # Controlar complexidade
        min_samples_split=20,       # Evitar splits muito específicos
        min_samples_leaf=10,        # Folhas maiores = mais generalização
        max_features='sqrt',        # Reduzir correlação entre árvores
        class_weight='balanced',    # Lidar com imbalance
        random_state=42,
        n_jobs=-1
    )
...
def treinar_modelo_3d_quadrantes_16_corrigido(history, games_today):
    """
    Treina modelo ML 3D CORRIGIDO para evitar overfitting e melhorar performance
    """
    ...
    y_home = history_clean["Target_AH_Binary"]
    y_away = 1 - y_home  # Target inverso para Away
    ...
    if not games_today.empty:
        # Garantir que games_today tem as mesmas colunas
        X_today = create_robust_features(games_today)
        
        # Garantir mesma estrutura que X_history
...
if not history.empty:
    ...
    if modelo_home is not None:
        ...
        # Mostrar estatísticas das previsões
        if 'Quadrante_ML_Score_Main' in games_today.columns:
            ...
            with col2:
                st.metric("🎯 Alto Confiança", high_confidence)
    else:
...
if not games_today.empty and 'Quadrante_ML_Score_Home' in games_today.columns:
    ...
    def estilo_tabela_3d_quadrantes(df):
        def cor_classificacao_3d(valor):
            ...
            elif 'VALUE' in str(valor): return 'background-color: #FFFACD; font-weight: bold'
            else: return ''
        
        colunas_para_estilo = []
        for col in ['Classificacao_Valor_Home', 'Classificacao_Valor_Away', 'Recomendacao']:
...
def resumo_3d_16_quadrantes_hoje(df):
    ...
    with col4:
        ...
        st.metric("🎯 Total Alto Valor", alto_valor_home + alto_valor_away)
