...
def treinar_modelo_inteligente(history, games_today):
    ...
    available_reg = [f for f in features_reg if f in history.columns]
    available_int = [f for f in features_int if f in history.columns]
    ...
    X_today = pd.concat([ligas_today, clusters_today, extras_today_3d, extras_today_reg, extras_today_int], axis=1)
...
def adicionar_indicadores_explicativos_3d_16_dual(df):
    ...
    def gerar_recomendacao(row):
        ...
        ml_side = row['ML_Side']
        if ml_side == 'HOME':
            return f"📈 HOME ({score_home:.1%})"
        else:
            return f"📈 AWAY ({score_away:.1%})"
