...
# 🔹 NOVO: lado visual (quem está “melhor” no plano Aggression × HandScore)
def infer_visual_side(row, angle_col='Quadrant_Angle_Normalized', dist_col='Quadrant_Dist',
                      min_angle: float = 15.0, min_dist: float = 5.0) -> str:
    """
    Define quem está melhor posicionado visualmente:
    - 'HOME'  → Home claramente acima/direita
    - 'AWAY'  → Away claramente acima/direita
    - 'BALANCED' → diferença pequena ou distância curta
    """
    ang = row.get(angle_col, np.nan)
    dist = row.get(dist_col, np.nan)
    if pd.isna(ang) or pd.isna(dist):
        return "BALANCED"
    if abs(ang) < min_angle or dist < min_dist:
        return "BALANCED"
    return "HOME" if ang > 0 else "AWAY"

games_today['Visual_Side'] = games_today.apply(infer_visual_side, axis=1)

st.dataframe(
    games_today[['Home','Away','Quadrant_Dist','Quadrant_Separation',
...
# No seu código principal, você pode usar:
...
def check_handicap_recommendation_correct(rec, handicap_result):
    ...
    if any(keyword in rec for keyword in ['HOME', 'VALUE NO HOME', 'FAVORITO HOME', 'MODELO CONFIA HOME']):
        return handicap_result == "HOME_COVERED"
    
    # Para recomendações AWAY  
    elif any(keyword in rec for keyword in ['AWAY', 'VALUE NO AWAY', 'FAVORITO AWAY', 'MODELO CONFIA AWAY']):
...
def calculate_handicap_profit(rec, handicap_result, odds_row, asian_line_decimal):
    ...
    def single_profit(result):
        ...
        if result == "PUSH":
            return 0
        elif (is_home_bet and result == "HOME_COVERED") or (is_away_bet and result == "AWAY_COVERED"):
            return odd  # vitória
        elif (is_home_bet and result == "AWAY_COVERED") or (is_away_bet and result == "HOME_COVERED"):
            return -1  # derrota
        return 0
...
def treinar_modelo_quadrantes_dual(history, games_today):
    ...
    extras = history[['Quadrant_Dist', 'Quadrant_Separation','Quadrant_Angle_Geometric',
                      'Quadrant_Angle_Normalized', 'CoverRate_Home', 'CoverRate_Away',
...
def adicionar_indicadores_explicativos_dual(df):
    ...
    # 3. RECOMENDAÇÃO DE APOSTA DUAL (com coerência geométrica)
    def gerar_recomendacao_dual(row):
        ...
        # Regras de “MODELO CONFIA” com checagem geométrica
        # Se o modelo está em HOME, mas visualmente AWAY é forte, só deixamos MODELO CONFIA se a confiança for muito alta
        if ml_side == 'HOME' and score_home >= 0.55:
            if visual_side == 'AWAY' and score_home < 0.75:
            ...
            if visual_side == 'HOME' and score_away < 0.75:
                return f'⚖️ ANALISAR (H:{score_home:.1%} A:{score_away:.1%})'
            return f'📈 MODELO CONFIA AWAY ({score_away:.1%})'

        # Casos de superavaliação de favorito
        if 'Market Overrates' in home_q and score_away >= 0.55:
...
if not games_today.empty and 'Quadrante_ML_Score_Home' in games_today.columns:
    ...
    colunas_possiveis = [
        ...
        'ML_Side', 'Visual_Side', 'Recomendacao', 'Asian_Line_Decimal',
