...
def calcular_rolling_wg_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcula features rolling dos Weighted Goals
    """
    df_temp = df.copy()
...
def enrich_games_today_with_wg(games_today, history):
    """
    Enriquece os jogos de hoje com as médias rolling do histórico
    """
    ...
    wg_cols = ['WG_Home_Team_Last', 'WG_AH_Home_Team_Last', 'WG_Away_Team_Last', 'WG_AH_Away_Team_Last']
    for col in wg_cols:
...
# ---------------- APLICAR WEIGHTED GOALS FEATURES ----------------
st.info("🧮 Calculando features de Weighted Goals...")

history = adicionar_weighted_goals(history)
history = adicionar_weighted_goals_ah(history)
history = calcular_rolling_wg_features(history)

games_today = adicionar_weighted_goals(games_today)
games_today = adicionar_weighted_goals_ah(games_today)
games_today = enrich_games_today_with_wg(games_today, history)

st.success(f"✅ Weighted Goals calculados: {len(history)} jogos históricos processados")
...
st.markdown("## 📊 Análise das Weighted Goals Features")
...
if not games_today.empty and 'WG_Diff' in games_today.columns:
    ...
    with col2:
        st.metric("Média WG_AH_Diff", f"{games_today['WG_AH_Diff'].mean():.3f}")
    with col3:
        st.metric("Confiança Média WG", f"{games_today['WG_Confidence'].mean():.1f}")
    
    fig, ax = plt.subplots(figsize=(10, 6))
    if 'Quadrante_ML_Score_Main' in games_today.columns:
        scatter = ax.scatter(games_today['WG_Diff'], games_today['WG_AH_Diff'], 
                            c=games_today['Quadrante_ML_Score_Main'],
                            cmap='RdYlGn', alpha=0.7, s=50)
        plt.colorbar(scatter, label='Score ML')
    else:
        ax.scatter(games_today['WG_Diff'], games_today['WG_AH_Diff'], alpha=0.7, s=50)
    
    ax.axhline(y=0, color='black', linestyle='-', alpha=0.3)
    ax.axvline(x=0, color='black', linestyle='-', alpha=0.3)
    ax.set_xlabel('WG_Diff (Home - Away)')
    ax.set_ylabel('WG_AH_Diff (Home - Away)')
    ax.set_title('Weighted Goals vs Performance ML')
    st.pyplot(fig)

st.markdown("### 🏆 Top Jogos por Weighted Goals")
if not games_today.empty and 'WG_Diff' in games_today.columns:
    top_wg = games_today.nlargest(10, 'WG_Diff')[['Home', 'Away', 'League', 'WG_Diff', 'WG_AH_Diff', 'WG_Confidence']]
...
for _, row in df_plot.iterrows():
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertext=(
            ...
            f"🎯 WG_Diff: {row.get('WG_Diff', 'N/A'):.3f}"
        ),
        showlegend=False
    ))
...
def treinar_modelo_quadrantes_dual(history, games_today):
    ...
    wg_features = history[['WG_Diff', 'WG_AH_Diff', 'WG_Confidence']].fillna(0)
    ...
    wg_today = games_today[['WG_Diff', 'WG_AH_Diff', 'WG_Confidence']].fillna(0)
...
st.markdown("---")
st.info("🎯 **Análise de Quadrantes ML Dual** - Sistema avançado para identificação de value bets em Home e Away baseado em Aggression × HandScore + Weighted Goals")