...
def main_handicap_v1():
    ...
    def market_cover_prob(row):
        ...
        if hc < 0:
            # Home favorito, precisa vencer bem → aproxima por p_home
            return p_h
        elif hc == 0:
            # Linha 0 → precisa vencer
            return p_h
        else:
            # Home recebe gols → aproxima como "não vitória forte do away"
            return 1 - p_a
    ...
    def gerar_ail_value_score_hibrido(df: pd.DataFrame, profile: str = "moderado") -> pd.DataFrame:
        ...
        for _, row in df.iterrows():
            ...
            # Handicap muito perto de 0 → PASS
            if abs(hc_home) < 0.25:
                picks.append("⚪ PASS")
                confs.append("PASS")
                sides.append("")
            ...
            # Lado SEMPRE baseado em P_Cover_Home_Especifico
            if p_home > 0.5:
                side = "HOME"
                hc_side = hc_home
            else:
