...
def main_handicap_v1():
    ...
    # Odds → probabilidades "fair" (removendo vig via normalização)
    ...
    def segmentar_por_handicap(df: pd.DataFrame, handicap_alvo: float, tolerancia: float = 0.25) -> pd.DataFrame:
        ...
        Sempre usando Asian_Line_Decimal (perspectiva HOME).
        """
    ...
    def market_cover_prob(row):
        ...
        # Aproximações:
        # HC < 0 → Home favorito, precisa vencer bem → usa p_home
        if hc < 0:
            return p_h
        # HC = 0 → precisa vencer → p_home
        elif hc == 0:
            return p_h
        # HC > 0 → Home recebe gols; aproxima como não vitória forte do Away
        else:
            return 1 - p_a  # ~ P(Home não perder forte)

    def calcular_edge_mercado(df: pd.DataFrame) -> pd.DataFrame:
        """
        Cria colunas:
          - Market_Cover_Prob (prob. MERCADO do HOME cobrir)
          - Edge_Model_vs_Market (P_model_HOME - P_market_HOME)
          - Edge_Pick (edge do lado escolhido na pick)
        """
        df = df.copy()
        ...
        # Edge do lado da pick:
        # Se pick é HOME → edge = Edge_Model_vs_Market
        # Se pick é AWAY → edge = -Edge_Model_vs_Market
    ...
    def aplicar_stamp(df: pd.DataFrame) -> pd.DataFrame:
        """
        Define AIL_Stamp = '🔷' para picks premium:
          - Confiança ALTA
          - AIL_Value_Score alto
          - Edge_Pick alto
        """
        df = df.copy()
        df['AIL_Stamp'] = ""
    ...
    def plot_model_vs_market(df: pd.DataFrame):
        """
        Gráfico comparando prob. do modelo vs prob. do mercado
        para o HOME cobrir o handicap, usando os maiores gaps.
        """
    ...
    if analise_modo == "📊 Análise Exploratória":
        ...
        # Tooltip para explicar o handicap do ponto de vista HOME
        ...
        if 'modelos_handicap' not in st.session_state:
            ...
            # Seção separada para análise visual Modelo vs Mercado
            with st.expander("📊 Análise de Valor: Modelo vs Mercado"):
                plot_model_vs_market(df_filtrado)
