...
def create_better_target_corrigido(df):
    """
    Cria targets MELHORADOS com múltiplas perspectivas
    """
    df = df.copy()

    # 1. Cálculo base do handicap
    df["Margin"] = df["Goals_H_FT"] - df["Goals_A_FT"]
    df["AH_Result"] = df.apply(
        lambda r: calc_handicap_result_corrigido(r["Margin"], r["Asian_Line_Decimal"]),
        axis=1
    )
    ...
    # 2. ✅ MULTIPLOS TARGETS
    # Target 1: Binário clássico (compatibilidade)
    df["Target_AH_Home"] = (df["AH_Result"] > 0.5).astype(int)

    # Target 2: Margem ajustada (regressão)
    df["Target_Margin"] = df["Margin"] + df["Asian_Line_Decimal"]
    
    # Target 3: Performance categorizada
    adjusted_margin = df["Target_Margin"]
    conditions = [
        adjusted_margin >= 2.0,    # 🏆 Dominante (+2 ou mais)
        adjusted_margin >= 1.0,    # ✅ Confortável (+1 a +2)  
        adjusted_margin >= 0.25,   # 👍 Vitória (+0.25 a +1)
        adjusted_margin >= -0.25,  # 🤝 Empate técnico (-0.25 a +0.25)
        adjusted_margin >= -1.0,   # 👎 Derrota (-1 a -0.25)
        adjusted_margin < -1.0     # 💀 Derrota pesada (-1 ou menos)
    ]
    choices = [5, 4, 3, 2, 1, 0]
    df["Target_Performance"] = np.select(conditions, choices, default=2)

    # 4. Análise de mercado
    df["Expected_Favorite"] = np.where(
        df["Asian_Line_Decimal"] < 0, "HOME",
    ...
    # Relatório completo
    st.info(f"🎯 Dataset final: {len(df)} jogos")
    st.info(f"📊 Target_AH_Home: {df['Target_AH_Home'].mean():.1%} positivos")
    st.info(f"📈 Target_Margin: média {df['Target_Margin'].mean():.2f} (std: {df['Target_Margin'].std():.2f})")
    
    # Distribuição de performance
    perf_counts = df['Target_Performance'].value_counts().sort_index()
    perf_labels = ['Péssimo', 'Ruim', 'Empate', 'Bom', 'Ótimo', 'Excelente']
    st.info("📋 Distribuição Performance:")
    for i, count in perf_counts.items():
        if i < len(perf_labels):
            st.write(f"   {perf_labels[i]}: {count} jogos ({count/len(df):.1%})")

    st.info(f"🦓 Zebras: {df['Zebra'].mean():.1%}")

    # Debug expandido
    debug_cols = [
        "League", "Home", "Away", "Asian_Line_Decimal",
        "Goals_H_FT", "Goals_A_FT", "Margin", "AH_Result",
        "Target_AH_Home", "Target_Margin", "Target_Performance", "Zebra"
    ]
    debug_cols = [c for c in debug_cols if c in df.columns]

    st.write("🔍 Amostra com múltiplos targets:")
...
def train_margin_regressor(X, y, feature_names):
    """Treina modelo de regressão para prever margem de vitória"""
...
# ---------------- MODELO ML 3D CORRIGIDO (MULTI-TARGET) ----------------
def treinar_modelo_3d_quadrantes_16_corrigido(history, games_today):
    """
    Treina modelos MULTIPLOS: classificação + regressão
    """
    st.markdown("## 🤖 TREINAMENTO MULTI-TARGET (Classificação + Regressão)")

    if history.empty:
        st.error("❌ Histórico vazio - não é possível treinar modelo")
        return None, None, None, games_today

    # 1. Criar múltiplos targets
    ...
    # 3. ✅ MULTIPLOS TARGETS
    y_ah_home = history_clean["Target_AH_Home"]      # Classificação binária
    y_ah_away = 1 - y_ah_home                       # Classificação complementar  
    y_margin = history_clean["Target_Margin"]        # Regressão contínua

    st.success(f"✅ Dados de treino: {X_history.shape[0]} amostras, {X_history.shape[1]} features")
    st.success(f"🎯 Targets: AH_Home({y_ah_home.mean():.1%}) | Margem(μ:{y_margin.mean():.2f})")

    # 4. Treinar modelos
    st.subheader("🎯 Modelo HOME (Classificação)")
    model_home = train_improved_model(X_history, y_ah_home, X_history.columns.tolist())

    st.subheader("🎯 Modelo AWAY (Classificação)")  
    model_away = train_improved_model(X_history, y_ah_away, X_history.columns.tolist())

    st.subheader("📈 Modelo MARGEM (Regressão)")
    model_margin = train_margin_regressor(X_history, y_margin, X_history.columns.tolist())

    # 5. Previsões para jogos de hoje
    if not games_today.empty:
        ...
        # Previsão de margem (regressão)
        predicted_margins = model_margin.predict(X_today)
        ...
        games_today['Predicted_Margin'] = predicted_margins

        # Lógica de aposta aprimorada com margem
        ...
        # ✅ NOVO: Confiança considerando MARGEM prevista
        margin_confidence = 1 / (1 + np.exp(-np.abs(predicted_margins) * 0.5))
        games_today['Combined_Confidence'] = 0.7 * games_today['Bet_Confidence'] + 0.3 * margin_confidence
        ...
        # Aposta aprovada (agora com confiança combinada)
        games_today['Bet_Approved'] = games_today['Combined_Confidence'] >= games_today['Min_Conf_Required']

        # Zebra considerando margem
        if 'Expected_Favorite' in games_today.columns:
            games_today['Is_Zebra_Bet'] = np.where(
                ...
                (np.abs(games_today['Predicted_Margin']) > 0.5),  # Margem significativa
                1, 0
            )
        else:
        ...
        # Label aprimorado
        def create_enhanced_bet_label(row):
            if not row['Bet_Approved']:
                return 'NO BET'
            
            base = 'BET HOME' if row['Bet_Side'] == 'HOME' else 'BET AWAY'
            margin = row['Predicted_Margin']
            
            if abs(margin) >= 1.5:
                strength = ' 🏆 FORTE'
            elif abs(margin) >= 1.0:
                strength = ' ✅ CONFORTAVEL'
            elif abs(margin) >= 0.5:
                strength = ' 👍 MODERADO'
            else:
                strength = ' 🤏 APERTADO'
                
            zebra = ' 🦓 ZEBRA' if row['Is_Zebra_Bet'] else ''
            
            return base + strength + zebra

        games_today['Bet_Label'] = games_today.apply(create_enhanced_bet_label, axis=1)

        st.success(f"✅ Previsões multi-target geradas para {len(games_today)} jogos")

        # Resumo executivo
        aprovados = games_today['Bet_Approved'].sum()
        zebras = games_today['Is_Zebra_Bet'].sum()
        avg_margin = games_today['Predicted_Margin'].mean()
        
        st.info(f"📊 Resumo: {aprovados} apostas aprovadas | {zebras} zebras | Margem média: {avg_margin:.2f}")

    return model_home, model_away, model_margin, games_today

# ---------------- EXECUÇÃO DO MODELO CORRIGIDO ----------------
if not history.empty:
    modelo_home, modelo_away, modelo_margin, games_today = treinar_modelo_3d_quadrantes_16_corrigido(history, games_today)

    if modelo_home is not None:
        st.success("✅ Modelo 3D multi-target treinado com sucesso!")
        ...
        if 'Quadrante_ML_Score_Main' in games_today.columns:
            ...
            with col2:
                st.metric("🎯 Apostas aprovadas", high_confidence)
            with col3:
                st.metric("📈 Margem Média", f"{avg_margin:.2f}")
    else:
...
def adicionar_indicadores_explicativos_3d_16_dual(df):
    ...
    def gerar_recomendacao_3d_16_dual(row):
        ...
        bet_conf = row.get('Combined_Confidence', 0.5)
        ...
        if is_zebra and expected_fav in ['HOME', 'AWAY']:
            return f'ZEBRA contra {expected_fav} ({bet_side}, Margem: {predicted_margin:.1f})'

        # Cenários fortes de favorito + margem alta
        if predicted_margin >= 1.5 and bet_side == 'HOME':
            return f'HOME dominante (Margem: {predicted_margin:.1f}, {bet_conf:.1%})'
        if predicted_margin <= -1.5 and bet_side == 'AWAY':
            return f'AWAY dominante (Margem: {predicted_margin:.1f}, {bet_conf:.1%})'

        # Valor com boa margem
        if abs(predicted_margin) >= 1.0 and bet_conf >= 0.60:
            return f'Boa margem + confiança ({bet_side}, {predicted_margin:.1f})'

        # Momentum negativo do lado oposto
        if momentum_h < -1.0 and bet_side == 'AWAY' and bet_conf >= 0.55:
            return f'HOME em má fase → AWAY ({predicted_margin:.1f})'
        if momentum_a < -1.0 and bet_side == 'HOME' and bet_conf >= 0.55:
            return f'AWAY em má fase → HOME ({predicted_margin:.1f})'

        return f'Analisar ({bet_side}, Margem: {predicted_margin:.1f}, {bet_conf:.1%})'

    df['Recomendacao'] = df.apply(gerar_recomendacao_3d_16_dual, axis=1)
    df['Ranking'] = df['Combined_Confidence'].rank(ascending=False, method='dense').astype(int)
...
if not games_today.empty and 'Quadrante_ML_Score_Home' in games_today.columns:
    ...
    # Ordenar pela confiança combinada
    ranking_3d = ranking_3d.sort_values('Combined_Confidence', ascending=False)

    colunas_3d = [
        'Ranking', 'League', 'Home', 'Away',
        'Goals_H_Today', 'Goals_A_Today',
        'Bet_Label', 'Bet_Side', 'Bet_Confidence', 'Combined_Confidence', 'Bet_Approved',
        'Expected_Favorite', 'Is_Zebra_Bet',
        'Quadrante_Home_Label', 'Quadrante_Away_Label',
        'Quadrante_ML_Score_Home', 'Quadrante_ML_Score_Away',
        'Predicted_Margin',  # ✅ NOVA COLUNA
        'Min_Conf_Required',
        'Recomendacao',
    ...
    def estilo_tabela_3d_quadrantes(df):
        # Gradiente nas probabilidades e margem
    ...
    aprovados = aprovados.sort_values('Combined_Confidence', ascending=False)
    ...
    if aprovados.empty:
        ...
        for _, row in aprovados.head(20).iterrows():
            ...
            with st.expander(titulo):
                ...
                st.write(f"**Aposta:** {row.get('Bet_Label', 'NO BET')}")
                st.write(f"**Lado ML:** {row.get('Bet_Side', '')} na linha {linha_str}")
                st.write(
                    f"**Confiança:** {row.get('Combined_Confidence', 0):.1%} "
                    f"(Prob: {row.get('Bet_Confidence', 0):.1%} | "
                    f"Margem: {row.get('Predicted_Margin', 0):.1f})"
                )
                st.write(f"**Threshold mínimo:** {row.get('Min_Conf_Required', 0):.1%}")
                st.write(f"**Favorito casa:** {row.get('Expected_Favorite', 'NONE')}")
...
def resumo_3d_16_quadrantes_hoje(df):
    ...
    if 'Predicted_Margin' in df.columns:
        margem_alta_positiva = len(df[df['Predicted_Margin'] >= 1.0])
        margem_alta_negativa = len(df[df['Predicted_Margin'] <= -1.0])
    else:
        margem_alta_positiva = margem_alta_negativa = 0

    aprovados = len(df[df['Bet_Approved'] == True]) if 'Bet_Approved' in df.columns else 0
    zebras = len(df[df['Is_Zebra_Bet'] == 1]) if 'Is_Zebra_Bet' in df.columns else 0
    ...
    with col1:
        ...
        st.metric("🎯 Apostas Aprovadas", aprovados)
    with col2:
        st.metric("📉 Momentum - Home", momentum_negativo_home)
        st.metric("📈 Momentum + Away", momentum_positivo_away)
        st.metric("🦓 Zebras", zebras)
    with col3:
        st.metric("📉 Momentum - Away", momentum_negativo_away)
        st.metric("📊 Margem Alta +", margem_alta_positiva)
        st.metric("🎯 Alto Valor Home", alto_valor_home)
    with col4:
        st.metric("📊 Margem Alta -", margem_alta_negativa)
        st.metric("📈 Confiança Média", f"{df['Combined_Confidence'].mean():.1%}" if 'Combined_Confidence' in df.columns else "N/A")
...
st.markdown("---")
st.success("🎯 **Sistema 3D de 16 Quadrantes ML MULTI-TARGET** implementado com sucesso!")
st.info("""
**Novas funcionalidades implementadas:**

✅ **Múltiplos Targets** - Classificação binária + regressão de margem  
✅ **Features Trigonométricas** - Seno/cosseno para direção 3D  
✅ **Confiança Combinada** - Probabilidade + margem prevista  
✅ **Labels Aprimorados** - Força da aposta baseada na margem  
✅ **Zebra com Margem** - Apenas zebras com margem significativa  
✅ **Resumo Executivo Expandido** - Mais métricas de performance  

Agora o sistema considera não apenas **SE** um time cobre, mas também **COM QUE MARGEM** - permitindo apostas mais inteligentes!
""")
