...
def calcular_market_opening_bias(df):
    """
    Calcula métricas baseadas nas odds de abertura (Odd_H_OP, Odd_D_OP, Odd_A_OP)
    e gera features adicionais para o modelo ML.
    """
    df = df.copy()
    if not all(col in df.columns for col in ['Odd_H_OP', 'Odd_D_OP', 'Odd_A_OP']):
        st.warning("⚠️ Odds de abertura não encontradas no dataset.")
        return df
    ...
    # Normaliza para somar 1 (retira o juice)
    ...
    for prefix in ['H', 'D', 'A']:
        df[f'Shift_Percent_{prefix}'] = (df[f'Odd_{prefix}'] - df[f'Odd_{prefix}_OP']) / df[f'Odd_{prefix}_OP']

    # Bias inicial do mercado (bookie vs modelo)
    ...
    # Indicador de viés (positivo = casa subestimou o time)
...
# Aplicar o cálculo nas duas bases
history = calcular_market_opening_bias(history)
games_today = calcular_market_opening_bias(games_today)

st.success("✅ Features de Market Opening Bias calculadas com sucesso!")
