...
if "cat_h" not in locals() or "cat_a" not in locals():
    st.error("âŒ Goal categories (BLOCK 4) not calculated.")
    st.stop()
...
def train_and_evaluate(X, y, name, num_classes):
    ...
    if not nan_stats.empty:
        st.markdown(f"### ðŸ“Š Missing values â€“ {name}")
...
st.markdown("## ðŸ“Š Results Analysis by Goal Category")
...
âšª = efficient team but low impact goals â†’ more draws.  
ðŸŸ¡ = inefficient team, but when they score it matters â†’ unstable profile.  
ðŸ”´ = inefficient team and irrelevant goals â†’ â€œlosingâ€ profile.  
""")