...
# =====================================================
# 📦 BLOCO 3️⃣ - CARREGAR DADOS
# =====================================================
st.subheader("📂 Carregando base de jogos")

df = pd.read_csv(os.path.join(GAMES_FOLDER, selected_file))
df = df.drop_duplicates(subset=["Date", "Home", "Away", "League"], keep="last").reset_index(drop=True)

# Garantir tipos corretos
df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
df = df.sort_values("Date").reset_index(drop=True)

st.write(f"✅ Jogos carregados: {len(df)} | Ligas únicas: {df['League'].nunique()}")


# =====================================================
# 🧩 BLOCO 3.1 - ENCODING TEMPORAL (por progresso da temporada)
# =====================================================
import numpy as np
import pandas as pd

def add_season_phase_encoding(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adiciona features sazonais cíclicas baseadas na posição do time dentro da temporada,
    usando o número de partidas já jogadas (Games_Home / Games_Away) e o tamanho da liga.

    Fórmula:
        Games_Per_Team = (N_teams - 1) * 2
        sin = sin(2π * Games_Played / Games_Per_Team)
        cos = cos(2π * Games_Played / Games_Per_Team)
    
    Retorna:
        df com colunas adicionais:
            ['Games_Per_Team', 'Games_Home_sin', 'Games_Home_cos',
             'Games_Away_sin', 'Games_Away_cos']
    """
    df = df.copy()

    required_cols = ['League', 'Home', 'Away', 'Games_Home', 'Games_Away']
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        st.warning(f"⚠️ Colunas ausentes para encoding temporal: {missing}")
        return df

    # -------------------------------
    # 1️⃣ Detectar número de times por liga
    # -------------------------------
    teams_per_league = (
        pd.concat([df[['League', 'Home']], df[['League', 'Away']].rename(columns={'Away': 'Home'})])
        .groupby('League')['Home']
        .nunique()
    ...
    # -------------------------------
    # 2️⃣ Calcular total de jogos por time na temporada
    # -------------------------------
    teams_per_league['Games_Per_Team'] = (teams_per_league['Num_Teams'] - 1) * 2
    teams_per_league['Games_Per_Team'] = teams_per_league['Games_Per_Team'].clip(lower=20)

    # -------------------------------
    # 3️⃣ Mesclar com o DataFrame original
    # -------------------------------
    df = df.merge(teams_per_league[['League', 'Games_Per_Team']], on='League', how='left')

    # -------------------------------
    # 4️⃣ Calcular seno e cosseno do progresso da temporada
    # -------------------------------
    def safe_angle(games_played, games_total):
        if pd.isna(games_played) or pd.isna(games_total) or games_total <= 0:
            return 0
        return 2 * np.pi * (games_played / games_total)

    df['Games_Home_Angle'] = df.apply(lambda x: safe_angle(x['Games_Home'], x['Games_Per_Team']), axis=1)
    df['Games_Away_Angle'] = df.apply(lambda x: safe_angle(x['Games_Away'], x['Games_Per_Team']), axis=1)

    df['Games_Home_sin'] = np.sin(df['Games_Home_Angle'])
    df['Games_Home_cos'] = np.cos(df['Games_Home_Angle'])
    df['Games_Away_sin'] = np.sin(df['Games_Away_Angle'])
    df['Games_Away_cos'] = np.cos(df['Games_Away_Angle'])

    df.drop(columns=['Games_Home_Angle', 'Games_Away_Angle'], inplace=True)

    st.info("✅ Codificação temporal cíclica adicionada com sucesso.")
    return df

# Aplicar codificação temporal
df = add_season_phase_encoding(df)


# =====================================================
# 📊 BLOCO 4️⃣ - FEATURE ENGINEERING
# =====================================================
...
# =====================================================
# 🧠 BLOCO 5️⃣ - SELECIONAR FEATURES PARA MODELO 3D
# =====================================================
st.subheader("🎯 Preparando features para modelo 3D")
...
features_modelo = features_base + features_temporais

X = df[features_modelo].fillna(0)

st.write(f"🧩 Features incluídas: {len(features_modelo)} → {features_modelo}")
