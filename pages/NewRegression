...
# ==============================================================
# 🧩 BLOCO – CLUSTERIZAÇÃO 3D (KMEANS) COM 8 CLUSTERS
# ==============================================================

def classificar_8_clusters_detalhado(df, centroids):
    """Classificação detalhada para os 8 clusters baseada nos centróides"""
    
    classificacoes = {}
    estrategias = {}
    
    for cluster_id in range(len(centroids)):
        dx, dy, dz = centroids[cluster_id]
        
        # 🎯 CLASSIFICAÇÃO AUTOMÁTICA BASEADA NOS CENTRÓIDES
        if dx > 0.6 and dy > 0.8 and dz > 0.8:
            classificacoes[cluster_id] = '🏆 HOME SUPER DOMINANTE'
            estrategias[cluster_id] = 'Aposta forte no Home, alto valor confirmado'
        elif dx < -0.6 and dy < -0.8 and dz < -0.8:
            classificacoes[cluster_id] = '🏆 AWAY SUPER DOMINANTE'
            estrategias[cluster_id] = 'Aposta forte no Away, alto valor confirmado'
        elif dx > 0.4 and dy > 0.3 and dz < 0:
            classificacoes[cluster_id] = '💪 HOME LIGA FORTE / TIME FRACO'
            estrategias[cluster_id] = 'Aposta moderada no Home, verificar momentum do time'
        elif dx < -0.4 and dy < -0.3 and dz > 0:
            classificacoes[cluster_id] = '💪 AWAY LIGA FORTE / TIME FRACO'
            estrategias[cluster_id] = 'Aposta moderada no Away, verificar momentum do time'
        elif abs(dx) < 0.3 and dy > 0.4 and dz > 0.4:
            classificacoes[cluster_id] = '⚖️ EQUILÍBRIO + MOMENTUM HOME'
            estrategias[cluster_id] = 'Value no Home, ótimo custo-benefício'
        elif abs(dx) < 0.3 and dy < -0.4 and dz < -0.4:
            classificacoes[cluster_id] = '⚖️ EQUILÍBRIO + MOMENTUM AWAY'
            estrategias[cluster_id] = 'Value no Away, ótimo custo-benefício'
        elif (dx > 0.5 and dy < -0.3) or (dx < -0.5 and dy > 0.3):
            classificacoes[cluster_id] = '🎭 CONFLITO AGGRESSION vs MOMENTUM'
            estrategias[cluster_id] = 'Analisar regressão à média, potencial value escondido'
        else:
            classificacoes[cluster_id] = '🌪️ PADRÃO MISTO/COMPLEXO'
            estrategias[cluster_id] = 'Analisar caso a caso, potencial alto risco'

    # Aplicar ao DataFrame
    df['Cluster3D_Desc'] = df['Cluster3D_Label'].map(classificacoes)
    df['Cluster3D_Estrategia'] = df['Cluster3D_Label'].map(estrategias)
...
def aplicar_clusterizacao_3d(df, n_clusters=8, random_state=42):
    ...
    Agora com 8 clusters e classificação detalhada.
    """
    df = df.copy()
    ...
    # 🎯 CLASSIFICAÇÃO DETALHADA DOS 8 CLUSTERS
    df = classificar_8_clusters_detalhado(df, kmeans.cluster_centers_)
    
    # 🧠 Calcular centroide de cada cluster para diagnóstico
    centroids = pd.DataFrame(kmeans.cluster_centers_, columns=['dx', 'dy', 'dz'])
    centroids['Cluster'] = range(n_clusters)
    centroids['Descricao'] = centroids['Cluster'].map(
        lambda x: df[df['Cluster3D_Label'] == x]['Cluster3D_Desc'].iloc[0] if len(df[df['Cluster3D_Label'] == x]) > 0 else 'N/A'
    )

    st.markdown("### 🧭 8 Clusters 3D Criados (KMeans)")
    st.dataframe(centroids.style.format({'dx': '{:.3f}', 'dy': '{:.3f}', 'dz': '{:.3f}'}))

    return df

def plot_8_clusters_3d(df):
    """Plot específico para 8 clusters com cores distintas"""
    
    cores_clusters = {
        0: 'darkblue',    # 🏆 HOME SUPER DOMINANTE
        1: 'darkred',     # 🏆 AWAY SUPER DOMINANTE  
        2: 'lightblue',   # 💪 HOME LIGA FORTE / TIME FRACO
        3: 'lightcoral',  # 💪 AWAY LIGA FORTE / TIME FRACO
        4: 'green',       # ⚖️ EQUILÍBRIO + MOMENTUM HOME
        5: 'lime',        # ⚖️ EQUILÍBRIO + MOMENTUM AWAY
        6: 'orange',      # 🎭 CONFLITO AGGRESSION vs MOMENTUM
        7: 'purple'       # 🌪️ PADRÃO MISTO/COMPLEXO
    }
    ...
    for cluster_id in range(8):
        ...
        if len(cluster_data) > 0:
            fig.add_trace(go.Scatter3d(
                ...
                z=cluster_data['dz'],
                mode='markers',
                name=f'Cluster {cluster_id}: {cluster_data["Cluster3D_Desc"].iloc[0]}',
                marker=dict(
                    size=8, 
                    color=cores_clusters.get(cluster_id, 'gray'),
                    opacity=0.8,
                    line=dict(width=1, color='white')
                ),
                hovertext=cluster_data.apply(
                    lambda r: f"{r['Home']} vs {r['Away']}<br>"
                             f"Cluster: {r['Cluster3D_Desc']}<br>"
                             f"Estratégia: {r['Cluster3D_Estrategia']}<br>"
                             f"dx: {r['dx']:.2f}, dy: {r['dy']:.2f}, dz: {r['dz']:.2f}", 
                    axis=1
                ),
                hoverinfo='text'
            ))
    ...
    fig.update_layout(
        title="🎯 8 Clusters 3D - Visualização Detalhada",
        scene=dict(
            xaxis_title='dx (Aggression Home - Away)',
            yaxis_title='dy (Momentum Liga Home - Away)',
            zaxis_title='dz (Momentum Time Home - Away)'
        ),
        height=700
    )
...
def analisar_performance_clusters(df):
    """Analisa performance histórica de cada cluster"""
    
    if df.empty or 'Quadrante_Correct' not in df.columns:
        st.info("📊 Dados de performance não disponíveis para análise")
        return pd.DataFrame()
    ...
    for cluster_id in range(8):
        ...
        if len(cluster_data) > 5:  # Mínimo de amostras
            total_jogos = len(cluster_data)
            jogos_com_resultado = cluster_data.dropna(subset=['Quadrante_Correct'])
            
            if len(jogos_com_resultado) > 0:
                win_rate = jogos_com_resultado['Quadrante_Correct'].mean()
                avg_profit = jogos_com_resultado['Profit_Quadrante'].mean() if 'Profit_Quadrante' in jogos_com_resultado else 0
                
                resultados.append({
                    'Cluster': cluster_id,
                    'Descricao': cluster_data['Cluster3D_Desc'].iloc[0],
                    'Total_Jogos': total_jogos,
                    'Jogos_Avaliados': len(jogos_com_resultado),
                    'Win_Rate': f"{win_rate:.1%}",
                    'Profit_Medio': f"{avg_profit:.3f}u",
                    'Estrategia': cluster_data['Cluster3D_Estrategia'].iloc[0] if 'Cluster3D_Estrategia' in cluster_data else 'N/A'
                })
    
    if resultados:
        return pd.DataFrame(resultados).sort_values('Win_Rate', ascending=False)
    else:
        return pd.DataFrame()
...
# ---------------- APLICAR CLUSTERIZAÇÃO 3D COM 8 CLUSTERS ----------------
st.markdown("## 🎯 Sistema de 8 Clusters 3D")

# Aplicar clusterização
history = aplicar_clusterizacao_3d(history, n_clusters=8)
games_today = aplicar_clusterizacao_3d(games_today, n_clusters=8)

# ---------------- VISUALIZAÇÃO DOS 8 CLUSTERS 3D ----------------
st.markdown("## 🎯 Visualização dos 8 Clusters 3D")

# Plot dos 8 clusters
fig_8_clusters = plot_8_clusters_3d(games_today)
st.plotly_chart(fig_8_clusters, use_container_width=True)

# Análise de performance dos clusters
st.markdown("### 📊 Performance dos Clusters (Base Histórica)")
performance_df = analisar_performance_clusters(history)
if not performance_df.empty:
    ...
    st.info("ℹ️ Execute alguns jogos para ver a performance dos clusters")

# Distribuição dos clusters hoje
st.markdown("### 📈 Distribuição dos Clusters Hoje")
