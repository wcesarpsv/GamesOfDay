...
if not games_today.empty and 'Quadrante_ML_Score_Home' in games_today.columns:
    ...
    # ============================================================
    # 🔄 CORREÇÃO DO SISTEMA MEI - VERSÃO SEGURA
    # ============================================================
    
    def classificar_mei_corrigido(mei, asian_line, ml_side):
        """Versão corrigida do MEI considerando linha asiática e lado"""
        if pd.isna(mei) or pd.isna(asian_line) or pd.isna(ml_side):
            return "⚪ Sem histórico"
        
        try:
            mei = float(mei)
            asian_line = float(asian_line)
            ml_side = str(ml_side).upper()
            
            # Determinar se é favorito ou underdog
            if ml_side == 'HOME':
                is_favorite = asian_line < -0.25
            else:  # AWAY
                is_favorite = asian_line > 0.25
    ...
    # PRIMEIRO: Garantir que a coluna MEI_Status existe
    if 'MEI_Home' in games_today.columns and 'Asian_Line_Decimal' in games_today.columns and 'ML_Side' in games_today.columns:
        games_today['MEI_Status'] = games_today.apply(
            lambda x: classificar_mei_corrigido(
                x['MEI_Home'], 
                x['Asian_Line_Decimal'], 
                x['ML_Side']
            ), 
            axis=1
        )
    else:
        st.warning("⚠️ Colunas necessárias para MEI não encontradas")
        games_today['MEI_Status'] = "⚪ Dados incompletos"
    
    # DEPOIS: Atualizar o ranking_quadrantes se ele já existe
    if 'ranking_quadrantes' in locals() and not ranking_quadrantes.empty:
        if 'MEI_Home' in ranking_quadrantes.columns and 'Asian_Line_Decimal' in ranking_quadrantes.columns and 'ML_Side' in ranking_quadrantes.columns:
            ranking_quadrantes['MEI_Status'] = ranking_quadrantes.apply(
                lambda x: classificar_mei_corrigido(
                    x['MEI_Home'], 
                    x['Asian_Line_Decimal'], 
                    x['ML_Side']
            ...
            ranking_quadrantes['MEI_Status'] = "⚪ Dados incompletos"
