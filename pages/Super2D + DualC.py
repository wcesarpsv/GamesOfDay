...
if not history.empty:
    ...
    history["Target_AH_Home"] = history.apply(
        lambda r: 1 if calc_handicap_result(r["Margin"], r["Asian_Line_Decimal"]) > 0.4 else 0, 
        axis=1
    )
    ...
    history["Target_AH_Away"] = history.apply(
        lambda r: 1 if calc_handicap_result(-r["Margin"], r["Asian_Line_Away"]) > 0.4 else 0,
        axis=1
    )
...
def debug_hcapzone_lookup(row, history, min_n=5):
    """
    Mostra exatamente os jogos do histórico que foram usados para calcular
    HcapZone_Score e HcapZone_N para a linha selecionada.
    """
    ...
    # Se tiver liga igual → priorizar liga primeiro
    sub_league = sub[sub['League'] == league]

    if len(sub_league) >= min_n:
        return "LEAGUE", sub_league

    # senão fallback global
    if len(sub) > 0:
        return "GLOBAL", sub
...
st.markdown("## 🧪 DEBUG - Verificação Handicap (Primeiras 50 linhas do Histórico)")
...
def debug_handicap_check(df_debug: pd.DataFrame):
    ...
    st.dataframe(subset[debug_cols], use_container_width=True)
