...
# 🔵 NOVO TARGET: SALDO DE GOLS (REGRESSÃO)
# Saldo de gols FULL TIME: Home - Away
history["Goal_Diff"] = history["Goals_H_FT"] - history["Goals_A_FT"]
...
def treinar_modelo_inteligente(history, games_today):
    """
    Treina:
      - Modelo ML de classificação: Target_AH_Home (HOME cobre o AH?)
      - Modelo ML de regressão: Goal_Diff (saldo de gols Home - Away)

    E adiciona ao games_today:
      - Prob_Home / Prob_Away / ML_Side / ML_Confidence
      - Pred_GoalDiff (regressão)
      - Pred_GoalDiff_minus_AH (margem esperada vs linha asiática)
    """
    # -------- 1) Features inteligentes (já existiam) --------
    history = adicionar_features_inteligentes_ml(history)
    games_today = adicionar_features_inteligentes_ml(games_today)

    # -------- 2) Features 3D + clusterização --------
    history = calcular_distancias_3d(history)
    games_today = calcular_distancias_3d(games_today)
    history = aplicar_clusterizacao_3d(history)
    games_today = aplicar_clusterizacao_3d(games_today)

    # -------- 3) HCR (Handicap Response Curve) --------
    games_today = adicionar_features_hcr(games_today, team_models, league_models)

    # -------- 4) One-hot de liga e cluster --------
    ligas_dummies = pd.get_dummies(history['League'], prefix='League')
    clusters_dummies = pd.get_dummies(history['Cluster3D_Label'], prefix='C3D')

    # -------- 5) Listas de features numéricas --------
    features_3d = [
        'Quadrant_Dist_3D', 'Quadrant_Separation_3D',
        'Quadrant_Sin_XY', 'Quadrant_Cos_XY',
        'Quadrant_Sin_XZ', 'Quadrant_Cos_XZ',
    ...
    # -------- 6) MATRIZ DE FEATURES X (comum p/ classif + regressão) --------
    X = pd.concat(
    ...
    # -------- 7) TARGETS --------
    # Classificação: cobre AH?
    y_home = history['Target_AH_Home'].astype(int)

    # Regressão: saldo de gols
    if 'Goal_Diff' not in history.columns:
        history['Goal_Diff'] = history['Goals_H_FT'] - history['Goals_A_FT']

    y_goal = history['Goal_Diff'].astype(float)

    # Alguns jogos podem estar sem gols → filtramos para regressão
    mask_reg = ~y_goal.isna()
    X_reg = X[mask_reg]
    y_goal_reg = y_goal[mask_reg]

    # -------- 8) MODELO DE CLASSIFICAÇÃO (igual antes) --------
    ...
    # -------- 9) MODELO DE REGRESSÃO (NOVO) --------
    model_goal = RandomForestRegressor(
        n_estimators=400,
        max_depth=12,
        random_state=42,
        n_jobs=-1
    ...
    if len(X_reg) > 50:
        model_goal.fit(X_reg, y_goal_reg)

    # -------- 10) Preparar X_today (mesma estrutura de X) --------
    ...
    X_today = pd.concat(
        [ligas_today, clusters_today, extras_today, extras_regressao_today, extras_inteligentes_today],
        axis=1
    )

    # -------- 11) PREVISÕES: CLASSIFICAÇÃO (AH HOME) --------
    ...
    # -------- 12) PREVISÕES: REGRESSÃO (SALDO DE GOLS) --------
    try:
        pred_goal_diff = model_goal.predict(X_today)
    except Exception:
        pred_goal_diff = np.zeros(len(games_today))
    ...
    inteligentes_no_top = len([
        f for f in importances.head(15).index
        if any(keyword in f for keyword in ['eh_', 'conflito_', 'momentum_', 'padrao_', 'score_'])
    ])
    st.info(f"🧠 Features Inteligentes no Top 15: {inteligentes_no_top}")

    st.success("✅ Modelo Inteligente (classificação + regressão) treinado com sucesso!")
    return model_home, model_goal, games_today
...
def update_real_time_data_3d(df):
    ...
    # ---------------------------------------
    # 🎯 Filtro por Score_Final_3D (0–100)
    # ---------------------------------------
    ...
    # ---------------------------------------
    # 🧮 Resultado do handicap (usa Goals_H_Today / Goals_A_Today)
    # ---------------------------------------
    df['Handicap_Result'] = df.apply(determine_handicap_result, axis=1)

    # Checagem de acerto (true/false/None)
...
# Executar treinamento 3D INTELIGENTE (Classificação + Regressão)
if not history.empty:
    modelo_home, modelo_goal, games_today = treinar_modelo_inteligente(history, games_today)
else:
