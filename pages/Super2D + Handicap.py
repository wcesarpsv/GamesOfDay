...
# ============================================================
# 🔥 FUNÇÕES LIVE SCORE - NOVAS E CORRETAS
# ============================================================

def determine_handicap_result_3d(row):
    if pd.isna(row['Goals_H_Today']) or pd.isna(row['Goals_A_Today']) or pd.isna(row['Asian_Line_Decimal']):
        return None

    handicap = row['Asian_Line_Decimal']
    diff = row['Goals_H_Today'] - row['Goals_A_Today']

    # Aplicação do handicap no placar
...
def calculate_handicap_profit_3d(row):
    ...
    if result == 'HOME_COVERED' or result == 'AWAY_COVERED':
        return odd - 1  # Win full
    elif result == 'HALF_WIN':
        return (odd - 1) / 2
    elif result == 'PUSH':
        return 0  # bet refunded
...
def check_1x2_correct(row):
    ...
    if recommended_side == 'HOME':
        return row['Result_1x2'] == 'HOME_WIN'
    else:
        return row['Result_1x2'] == 'AWAY_WIN'


def calculate_profit_1x2(row):
...
def update_real_time_data_1x2(df):
    df['Correct_1x2'] = df.apply(check_1x2_correct, axis=1)
    df['Profit_1x2'] = df.apply(calculate_profit_1x2, axis=1)
    return df


def generate_live_summary_3d(df):
    df_finalizados = df[df['status'] == 'FT']

    summary = {
        'Total Jogos': len(df),
        'Jogos Finalizados': len(df_finalizados),
        'Apostas Quadrante': df_finalizados['Quadrante_Correct'].count(),
        'Acertos Quadrante': df_finalizados['Quadrante_Correct'].sum(),
        'Winrate Quadrante': f"{(df_finalizados['Quadrante_Correct'].mean() * 100):.1f}%" if len(df_finalizados) else "0%",
        'Profit Quadrante': f"{df_finalizados['Profit_Quadrante'].sum():.2f}u",
        'ROI Quadrante': f"{(df_finalizados['Profit_Quadrante'].mean() * 100):.1f}%" if df_finalizados['Quadrante_Correct'].count() else "0%"
    }
...
def build_hcapzone_tables(history: pd.DataFrame,
    ...
    - Usa resultado REAL de handicap (com calc_handicap_result)
    - Considera apenas linhas entre min_line e max_line (default: -1.5 a +1.5)
    - Bins de handicap em quartos (0.25)
    - Para HOME: usa Asian_Line_Decimal (linha da casa)
    - Para AWAY: usa -Asian_Line_Decimal (linha do visitante)
    """
    ...
    # Somente jogos com info completa e em range
    df = df.dropna(subset=[
    ...
    df['Asian_Line_Bin_Home'] = (df['Asian_Line_Decimal'] * 4).round() / 4
    df['Asian_Line_Bin_Away'] = (df['Asian_Line_Away'] * 4).round() / 4

    # ---------------- GLOBAL – HOME ----------------
    global_home = (
        df.groupby(['Quadrante_Home', 'Asian_Line_Bin_Home'])['Cover_Home']
        .agg(['mean', 'size'])
        .reset_index()
        .rename(columns={
            'Quadrante_Home': 'Quadrante',
            'Asian_Line_Bin_Home': 'Asian_Line_Bin',
            'mean': 'CoverRate',
            'size': 'N'
        })
    )
    ...
    # ---------------- GLOBAL – AWAY ----------------
    global_away = (
        df.groupby(['Quadrante_Away', 'Asian_Line_Bin_Away'])['Cover_Away']
        .agg(['mean', 'size'])
        .reset_index()
        .rename(columns={
            'Quadrante_Away': 'Quadrante',
            'Asian_Line_Bin_Away': 'Asian_Line_Bin',
            'mean': 'CoverRate',
            'size': 'N'
        })
    )
    ...
    if 'League' in df.columns:
        league_home = (
            df.groupby(['League', 'Quadrante_Home', 'Asian_Line_Bin_Home'])['Cover_Home']
            .agg(['mean', 'size'])
            .reset_index()
            .rename(columns={
                'Quadrante_Home': 'Quadrante',
        ...
        league_away = (
            df.groupby(['League', 'Quadrante_Away', 'Asian_Line_Bin_Away'])['Cover_Away']
            .agg(['mean', 'size'])
            .reset_index()
            .rename(columns={
                'Quadrante_Away': 'Quadrante',
        ...
        league_home = pd.DataFrame(columns=['League', 'Quadrante', 'Asian_Line_Bin', 'CoverRate', 'N'])
        league_away = pd.DataFrame(columns=['League', 'Quadrante', 'Asian_Line_Bin', 'CoverRate', 'N'])
...
if not games_today.empty and 'Quadrante_ML_Score_Home' in games_today.columns:
    ...
    live_summary = generate_live_summary_3d(ranking_quadrantes)
